...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith('admin|'):
        ...
        if not order:
            ...
            return
        if action == 'confirm':
            await update_order_status(order_id, STATUS_CONFIRMED)
            ...
            await update_order_status(order_id, STATUS_REJECTED)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('admin_send_order') and text:
        ...
        await update_order_status(order_id, STATUS_SENT)
...
# ایجاد اپلیکیشن تلگرام (برای استفاده در Webhook)
application = None
...
def webhook():
    ...
    if application is None:
        ...
        # لازم نیست start/stop کنیم، فقط پردازش می‌کنیم
...
@app.route('/')
def home():
    orders_count = len(get_user_orders(0))  # dummy
    ...
    <body style="background:#0f0c29;color:#fff;font-family:sans-serif;display:flex;justify-content:center;align-items:center;min-height:100vh;margin:0;">
        <div style="background:rgba(255,255,255,0.05);border-radius:28px;padding:40px;max-width:500px;text-align:center;border:1px solid rgba(255,255,255,0.1);">
            ...
            <a href="/setwebhook" class="btn" style="background:#4f46e5;color:#fff;padding:12px 24px;border-radius:40px;text-decoration:none;display:inline-block;margin:5px;">⚙️ تنظیم Webhook</a>
            <a href="/webhook-info" class="btn" style="background:transparent;border:1px solid #4f46e5;color:#4f46e5;padding:12px 24px;border-radius:40px;text-decoration:none;display:inline-block;margin:5px;">📡 اطلاعات Webhook</a>
...
# ==================== اجرا (برای توسعه محلی) ====================
if __name__ == '__main__':
    init_db()
    app.run(host='0.0.0.0', port=int(os.environ.get('PORT', 5000)))
else:
    # در محیط Railway، فقط دیتابیس را آماده می‌کنیم
    init_db()
